// Creates an event on the signed-in user's primary Google Calendar, with an
// auto-generated Meet link and email invites sent to the attendees (Google
// handles that via sendUpdates=all — JetFlow never sends email itself).
// Same token-refresh flow as google-calendar-events: refresh token and
// client secret never leave the server.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!tokenRow) {
      return jsonResponse({ error: "Conecte sua Google Agenda antes de criar uma reunião.", reauthRequired: true });
    }
    ...
    if (!createRes.ok) {
      const reason = createData.error?.errors?.[0]?.reason || "";
      const reauthRequired = createRes.status === 403 && /insufficient/i.test(reason || createData.error?.message || "");
      return jsonResponse(
        { error: createData.error?.message || "Erro ao criar reunião na Google Agenda.", reauthRequired },
        reauthRequired ? 200 : 502
      );
    }

    let meetLink: string | null = createData.hangoutLink || null;
    if (!meetLink && createData.conferenceData?.entryPoints) {
      const videoEntry = createData.conferenceData.entryPoints.find((p: any) => p.entryPointType === "video");
